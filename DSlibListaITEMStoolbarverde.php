	...
	if (VerificaConBD()) {			
		...
		$resultad1 = mysql_query($sq1, $_SESSION['BD']);	
			while ($row1 = mysql_fetch_array($resultad1)) {	
								...
								$l='<a style ="color:green ; font: bold 8.5px tahoma,arial,verdana,sans-serif;">';
...
function DevuelveListaMenuItem1() { 
	...
	while ($row = mysql_fetch_array($resultadohoras)){
		...
		$resultad = mysql_query($sq, $_SESSION['BD']);	
			while ($row = mysql_fetch_array($resultad)) {		
						...
						// $l='<a style =" font: bold 9px tahoma,arial,verdana,sans-serif;">';   <img src=".$img."/>
								...
								{
									 $cmenu = $cmenu. "{	title: '<B style =color:green ><font size=1>". $row['MENU']. "</font></B>',".
								           "    layout: 'absolute',".
										   "    frame: true,".
										    "  id:'Me".$row['COD_MENU']."',".
		...
		$l='<a style ="color:#green; font: bold 7.5px tahoma,arial,verdana,sans-serif;">';
		 $l1='<a style ="color:#green; font: bold 8.5px tahoma,arial,verdana,sans-serif;">';
