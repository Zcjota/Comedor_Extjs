...
Ext.onReady(function(){
	...
	Ext.dsdata.storeSubcentro = new Ext.data.JsonStore({  	
		url: '../servicesAjax/DSListadoSubcentroAjax.php',  
		root: 'data',   
		totalProperty: 'total',
	...
	var Paginas = new Ext.PagingToolbar({
		pageSize: 25, 
		store: Ext.dsdata.storeSubcentro,
		displayInfo: true,
		afterPageText: 'de {0}',
		beforePageText: 'Pag.',
	...
	var bfilter = new Ext.Toolbar.Button({
		...
		handler: function(btn,e) {					
			...
			{  
				...
				Ext.dsdata.storeSubcentro.baseParams['buscar'] = filterVal;
				Ext.dsdata.storeSubcentro.reload({params:o});
			} 
			else {
				Ext.dsdata.storeSubcentro.clearFilter();
			}	
		}
	});
	...
	{
        cod = Ext.dsdata.storeSubcentro.getAt(indice).get('codigo'); 
		Ext.Ajax.request({  
		url: '../servicesAjax/DSdesactivarSubcentroAJAX.php',  
		method: 'POST',  
		params: {id:cod},  
		success: desactivo,  
