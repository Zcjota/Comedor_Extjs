...
Ext.onReady(function(){
	...
	Ext.dsdata.storeReloj = new Ext.data.JsonStore({  	
		url: '../servicesAjax/DSListadoRelojComedorAjax.php',  
		root: 'data',   
		totalProperty: 'total',
		fields: ['codigo', 'descripcion','ip','oct1','oct2','oct3','oct4']		
	}); 
	var indice=-1;
	var Paginas = new Ext.PagingToolbar({
		pageSize: 25, 
		store: Ext.dsdata.storeReloj,
		displayInfo: true,
		afterPageText: 'de {0}',
		beforePageText: 'Pag.',
	...
	var Columnas = new Ext.grid.ColumnModel(  
		...
		,{  
			header: '<a style ="color:#15428B; font: bold 11px tahoma,arial,verdana,sans-serif;">IP</a>', 
			dataIndex: 'ip',  
			renderer: formato, 
			width: 100,
			align: 'left',
	...
	var filter = new Ext.form.TextField({name: 'filterValue',cls: 'name',
	...
	var bfilter = new Ext.Toolbar.Button({
		...
		handler: function(btn,e) {					
			...
			{  
				...
				Ext.dsdata.storeReloj.baseParams['buscar'] = filterVal;
				Ext.dsdata.storeReloj.reload({params:o});
			} 
			else {
				Ext.dsdata.storeReloj.clearFilter();
			}	
		}
	});
	...
	{
        cod = Ext.dsdata.storeReloj.getAt(indice).get('codigo'); 
		Ext.Ajax.request({  
		url: '../servicesAjax/DSdesactivarRelojComedorAJAX.php',  
		method: 'POST',  
		params: {id:cod},  
		success: desactivo,  
