...
function DevuelveListaMenuItem() { 
	...
	while ($row = mysql_fetch_array($resultadosr)){
		...
		$data = '';
			...
			//'<a style ="color:##white ; font: bold 8px tahoma,arial,verdana,sans-serif;">APROBADOR</a>',  
				...
				$valor_menu = $row['COD_SUB_MENU'] * 1;	
						...
						if (strcmp($pmenu, $row['COD_MENU']) == 0) {				
							  if (strlen($cmenu) == 0) {
									...
									{
										$cmenu = "{	xtype: 'treepanel', title:'<FONT FACE=ARIAL  SIZE=1> <b style = color:red; >" . $row['MENU'] . "          <img src=".$img."/></b></FONT>', iconCls:'MAN1',	id: 'MENU" . $row['COD_MENU'] . 
										"',	margins: '2 2 0 2', autoScroll: true, rootVisible: false, collapsed: true, root: new Ext.tree.AsyncTreeNode({ children: ["; 
									}
									else
									{
										$cmenu = "{	xtype: 'treepanel', title:'<FONT FACE=ARIAL  SIZE=1> <b style = color:white ; >" . $row['MENU'] . "         </b></FONT>', iconCls:'MAN1',	id: 'MENU" . $row['COD_MENU'] . 
										"',	margins: '2 2 0 2', autoScroll: true, rootVisible: false, collapsed: true, root: new Ext.tree.AsyncTreeNode({ children: ["; 
									}
									
							}
							if($row['COD_SUB_MENU']==25 and $ttipo>0)
							  $rmenu .= " {text: '<FONT FACE=ARIAL  SIZE=0><b style = color:red; >" . $titulo . "</b></FONT>', leaf: true, id:'s" . $row['COD_SUB_MENU'] . "', url: '" . $row['RUTA'] . "', iconCls:'submenugris'},";
							else{
									if(($row['COD_SUB_MENU']==26 and $ttipo1>0) or ($row['COD_SUB_MENU']==45 and $cont>0) or ($row['COD_SUB_MENU']==24 and $ttipob1>0) or ($row['COD_SUB_MENU']==29 and $ttipor1>0) or ($row['COD_SUB_MENU']==15 and $total_sin_revision>0) or ($row['COD_SUB_MENU']==63 and $total_sin_revisionretraso>0))
									 $rmenu .= " {text: '<FONT FACE=ARIAL  SIZE=0><b style = color:red; >" . $titulo . " </b></FONT>', leaf: true, id:'s" . $row['COD_SUB_MENU'] . "', url: '" . $row['RUTA'] . "', iconCls:'submenugris'},";
									  ...
									  $rmenu .= " {text: '<FONT FACE=ARIAL  SIZE=0><b style = color:white ; >" . $titulo . "</b></FONT>', leaf: true, id:'s" . $row['COD_SUB_MENU'] . "', url: '" . $row['RUTA'] . "', iconCls:'submenugris'},";
							 }
					  	}	
						else {
									if (strcmp($pmenu, '') != 0) {
